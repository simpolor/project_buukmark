...
<tiles:insertDefinition name="admin-layout">
    <tiles:putAttribute name="admin-body">
		...
		<script type="text/javascript">
			...
			function f_modify(){
				$("#noticeViewForm").attr("action", "${pageContext.request.contextPath}/admin/notice/modify");
				$("#noticeViewForm").attr("method", "get");
				$("#noticeViewForm").submit();
			}
			
			function f_delete(){
				if(confirm("등록한 공지사항을 삭제하시겠습니까?")){
					$("#noticeViewForm").attr("action", "${pageContext.request.contextPath}/admin/notice/delete");
					$("#noticeViewForm").attr("method", "post");
					$("#noticeViewForm").submit();
				}
			}
			...
			function f_replyCancel(replySeq){
				var memberId = '${sessionScope.SESSION_MEMBER_ID}';
				var boardSeqId = '#notice_seq_'+replySeq;
				var replySeqId = '#reply_seq_'+replySeq;
				var replyContentId = '#reply_content_'+replySeq
				var replyListGroupId = '#reply_list_group_'+replySeq;
				...
				html += '	</div>';
				html += '	<div class="reply-list-body">';
				html += '		<input type="hidden" id="notice_seq_'+replySeq+'" value="'+$(boardSeqId).val()+'" />';
				html += '		<input type="hidden" id="reply_seq_'+replySeq+'" value="'+$(replySeqId).val()+'" />';
		...
		<div id="section">
			<div class="notice-nav">
				<div class="notice-nav-title">
					<span>(관리자) 공지사항 상세보기</span>
				</div>
			</div>
			
			<div class="notice-view">
				<form id="noticeViewForm">
					<input type="hidden" id="notice_seq" name=notice_seq value="${noticeVO.notice_seq}" />
					<table>
			...
			<div class="notice-btn">
				<div class="notice-btn-list">
					<div class="notice-btn-group">
						...
						<a href="${pageContext.request.contextPath}/admin/notice/list">목록</a>
					</div>
				</div>
			</div>
