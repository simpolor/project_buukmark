    ...
    <tiles:putAttribute name="user-body">
		...
		<script type="text/javascript">
			...
			function f_del( bookmarkSeq ){
				if(confirm("즐겨찾기를 삭제하시겠습니까?")){
					...
					$("#bookmarkForm").attr("action", "${pageContext.request.contextPath}/bookmark/del");
					$("#bookmarkForm").attr("method", "post");
					$("#bookmarkForm").submit();
				}
			}
			
			function f_myCancel( bookmarkSeq ){
				if(confirm("즐겨찾기를 취소하시겠습니까?")){
					$("#bookmark_seq").val(bookmarkSeq);
					$("#bookmarkForm").attr("action", "${pageContext.request.contextPath}/bookmark/myCancel");
					$("#bookmarkForm").attr("method", "post");
					$("#bookmarkForm").submit();
				}
			}
		...
		<div id="section">
			<div class="bookmark-mySearch">
				<form:form id="bookmarkSearchForm" action="${pageContext.request.contextPath}/bookmark/myList" method="get" commandName="bookmarkVO" style="margin: 0;" >
					<fieldset>
					...
					<fieldset>
						...
						<p>
							<form:select path="bookmark_type" class="bookmark-add-type">
								...
								<form:option value="포털사이트">포털사이트</form:option>
					...
					<table>
						...
						<tbody>
							<c:choose>
								 			...
								 			<td align="center">
												<c:if test="${! empty sessionScope.SESSION_MEMBER_ID}">
													<c:choose>	
									 					...
									 					<c:otherwise>
								 							<i class="fa fa-star color-orange corsor" aria-hidden="true" onclick="f_myCancel('${bookmarkList.bookmark_seq}'); return false;"></i>
									 					</c:otherwise>
									 				</c:choose>
									 			</c:if>
											</td>
								...
								<c:otherwise>
									<tr>
							 			<td colspan="4" align="center">등록된 즐겨찾기가 없습니다.</td>
							 		</tr>
								</c:otherwise>
							</c:choose>
						</tbody>
			...
			<div class="bookmark-pagination">
				<a href="${pageContext.request.contextPath}/bookmark/myList?page=${paging.startPage}"><i class="fa fa-angle-double-left"></i></a>
				<a href="${pageContext.request.contextPath}/bookmark/myList?page=${paging.prevPage}"><i class="fa fa-angle-left"></i></a>
				...
				<a href="${pageContext.request.contextPath}/bookmark/myList?page=${paging.nextPage}"><i class="fa fa-angle-right"></i></a>
				<a href="${pageContext.request.contextPath}/bookmark/myList?page=${paging.endPage}"><i class="fa fa-angle-double-right"></i></a>
